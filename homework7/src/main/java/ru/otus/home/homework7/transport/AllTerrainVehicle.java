...
public class AllTerrainVehicle extends Transport {
    ...
    public AllTerrainVehicle(int gasoline, Human driver) {
    ...
    public boolean move(int distance, Land landType) {
        ...
        if (ArrayUtils.contains(forbiddenLand, landType))
            throw new IllegalArgumentException("Forbidden land");
        else if (distance > gasoline)
            throw new IllegalArgumentException("There is not enough gasoline in the vehicle");
        else {
